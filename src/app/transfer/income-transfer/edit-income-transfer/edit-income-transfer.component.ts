...
import {
  SyncAutoCompleteModel,
  SyncButtonModel,
  SyncDropdownlistModel,
  SyncNumericTextBoxModel,
  SyncTextBoxModel,
} from "@shared/user-interface/dropdownlist/sync-models";
  ...
  implements OnInit, AfterViewInit
{
  ...
  initialIncomeTransfers() {
    ...
    let faxNumber =
      routeData?.number == "undefined" ? undefined : routeData?.number;
    let companyId =
      routeData?.companyId == "undefined" ? undefined : routeData?.companyId;

    this._incomeTransferService
      .getForEdit(routeData?.fromDate, routeData?.toDate, companyId, faxNumber)
      .subscribe((result) => {
  ...
  generateRow(index, data) {
    ...
    if (data != undefined) {
      percentageModel.value = data.amount
        ? (100 * data.commission) / data.amount
        : 0;
    }
    ...
    switch (data?.paymentType) {
      ...
      default: {
        ...
        let beneficiary = this.customers.find(
          (x) => x.id == data?.beneficiaryId
        );
  ...
  generateNumeric(model) {
    ...
    if (model.placeholder != undefined) numeric.placeholder = model.placeholder;
  ...
  generateِTextBox(model) {
    ...
    if (model.placeholder != undefined) input.placeholder = model.placeholder;
  ...
  public onchangePaymentType: EmitType<ChangeEventArgs> = (
    ...
    let index = e.element.getAttribute("id").replace("paymentType", "");
    ...
    let incomeTransferDetail =
      this.searchResult[this.index]?.incomeTransferDetails[index];
    switch (e.value) {
      case 0: {
        let parentDiv = this.generateِDiv(tdBeneficiary, "row");

        let beneficiary = this.customers.find(
          (x) => x.id == incomeTransferDetail?.beneficiaryId
        );
        let beneficiaryDiv = this.generateِDiv(parentDiv, "col-md-12");
        var beneficiaryModel = new SyncAutoCompleteModel({
          parentEle: beneficiaryDiv,
  ...
  public onchangeCurrency: EmitType<ChangeEventArgs> = (
    e: ChangeEventArgs
  ) => {};

  public onchangeBeneficiary: EmitType<ChangeEventArgs> = (
    e: ChangeEventArgs
  ) => {
    ...
    let beneficiaryPhoneEle = document.getElementById(
      eleId
    ) as HTMLInputElement;
    if (phoneNumber) beneficiaryPhoneEle.setAttribute("value", phoneNumber);
  };
  ...
  public onchangeSender: EmitType<ChangeEventArgs> = (e: ChangeEventArgs) => {
    ...
    if (phoneNumber) senderPhoneEle.setAttribute("value", phoneNumber);
  };

  public onchangeCompany: EmitType<ChangeEventArgs> = (e: ChangeEventArgs) => {
    this._companyService.getAllBalances(Number(e.value)).subscribe((result) => {
      this.companyBalances = result;
    });
  };

  public onchangeToClient: EmitType<ChangeEventArgs> = (
    e: ChangeEventArgs
  ) => {};
  public onchangeToCompany: EmitType<ChangeEventArgs> = (
    e: ChangeEventArgs
  ) => {};
  public onchangeCommission: EmitType<any> = (e: any) => {
    ...
    if (input != undefined) {
      ...
      let percentage = getInstance(
        document.getElementById(percentageId),
        NumericTextBox
      ) as NumericTextBox;
      let amount = getInstance(
  ...
  public onchangePersentege: EmitType<any> = (e: any) => {
    ...
    if (input != undefined) {
      ...
      let commission = getInstance(
        document.getElementById(commissionId),
        NumericTextBox
      ) as NumericTextBox;
      let amount = getInstance(
        document.getElementById(amountId),
  ...
  validateRow(index) {
    ...
    if (currency.value == undefined) {
      validationResult.fields.push({
        name: "currency",
        message: this.l("TheField{0}IsRequired", "Currency"),
      });
    } else {
    ...
    if (amount.value == undefined) {
      validationResult.fields.push({
        name: "amount",
        message: this.l("TheField{0}IsRequired", "Amount"),
      });
    } else {
    ...
    if (commission.value == undefined) {
      validationResult.fields.push({
        name: "commission",
        message: this.l("TheField{0}IsRequired", "Commission"),
      });
    } else {
    ...
    if (sender.value == undefined) {
      validationResult.fields.push({
        name: "sender",
        message: this.l("TheField{0}IsRequired", "Sender"),
      });
    } else {
    ...
    let senderPhoneEle = document.getElementById(
      senderPhoneId
    ) as HTMLInputElement;
    ...
    let paymentType = getInstance(
      document.getElementById(paymentTypeId),
      DropDownList
    ) as DropDownList;
    if (paymentType.value == undefined) {
      validationResult.fields.push({
        name: "paymentType",
        message: this.l("TheField{0}IsRequired", "PaymentType"),
      });
    } else {
    ...
    if (incomeTransferDetail.paymentType == 0) {
      ...
      let beneficiaryEle = getInstance(
        document.getElementById(beneficiaryId),
        AutoComplete
      ) as AutoComplete;
      if (beneficiaryEle.value == undefined) {
        validationResult.fields.push({
          name: "beneficiary",
          message: this.l("TheField{0}IsRequired", "Beneficiary"),
        });
      } else {
    ...
    if (incomeTransferDetail.paymentType == 1) {
      ...
      let toClient = getInstance(
        document.getElementById(toClientId),
        DropDownList
      ) as DropDownList;
      if (toClient.value == undefined) {
        validationResult.fields.push({
          name: "toClient",
          message: this.l("TheField{0}IsRequired", "ToClient"),
        });
      } else {
      ...
      let clientCommissionEle = getInstance(
        document.getElementById(clientCommissionId),
        NumericTextBox
      ) as NumericTextBox;
      if (clientCommissionEle.value != undefined) {
        incomeTransferDetail.clientCommission = Number(
          clientCommissionEle.value
        );
      }
    }
    ...
    if (incomeTransferDetail.paymentType == 2) {
      ...
      let toCompany = getInstance(
        document.getElementById(toCompanyId),
        DropDownList
      ) as DropDownList;
      if (toCompany.value == undefined) {
        validationResult.fields.push({
          name: "toCompany",
          message: this.l("TheField{0}IsRequired", "ToCompany"),
        });
      } else {
      ...
      let companyCommissionEle = getInstance(
        document.getElementById(companyCommissionId),
        NumericTextBox
      ) as NumericTextBox;
      if (companyCommissionEle.value != undefined) {
        incomeTransferDetail.companyCommission = Number(
          companyCommissionEle.value
        );
      }
    }
  ...
  checkCurrencyBalanceIfExist(data) {
    let index = this.companyBalances.findIndex(
      (x) =>
        x.companyId == this.incomeTransfer.companyId &&
        x.currencyId == data.currencyId
    );
  ...
  showSearchIncomeDialog() {
    this._modalService
      .open(SearchIncomeTransferComponent)
