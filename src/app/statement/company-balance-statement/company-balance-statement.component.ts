...
export class CompanyBalanceStatementComponent
  ...
  implements OnInit
{
  ...
  initialCurrentBalance(companyId, currencyId, fromDate, toDate) {
    this._companyCashFlowService
      .getCurrentBalance(companyId, currencyId, fromDate, toDate)
      .subscribe((result) => {
  ...
  showMatchingDialog(args) {
    this._modalService
      .open(CompanyMatchingDialogComponent, {
        context: {
